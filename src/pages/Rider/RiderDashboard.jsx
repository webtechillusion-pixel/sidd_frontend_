...
import CustomerRiderChat, {
  ChatButton,
} from "../../components/CustomerRiderChat";
...
export default function RiderDashboard() {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await riderService.getNotifications({
        page: 1,
        limit: 10,
      });
      if (response.success && response.data) {
        ...
        const unread =
          response.data.notifications?.filter((n) => !n.isRead).length || 0;
  ...
  useEffect(() => {
    ...
    if (socket && isConnected && user?._id) {
      ...
      const handleNewBookingRequest = (bookingData) => {
        ...
        if (isMountedRef.current && onlineStatusRef.current) {
          ...
          const transformedBooking = {
            ...
            eta: "5 min",
            rating:
              bookingData.user?.rating ||
              bookingData.customerInfo?.rating ||
              4.5,
            details: {
              ...bookingData,
          ...
          console.log(
            "⚠️ Booking ignored - rider offline or component unmounted",
          );
        }
      };
      ...
      const handleGlobalBookingRequest = (bookingData) => {
        ...
        if (
          profileData.cab?.cabType === bookingData.vehicleType ||
          !profileData.cab
        ) {
      ...
      const handleBookingTaken = (data) => {
        if (isMountedRef.current) {
          ...
          setRideRequests((prev) =>
            prev.filter((r) => r.id !== data.bookingId),
          );
            ...
            (rider.completedRides > 0
              ? Math.min(
                  ...
                  (rider.completedRides /
                    (rider.completedRides + (rider.rejectedRides || 0))) *
                    100,
                )
              : 0),
  ...
  const fetchEarnings = async () => {
    try {
      ...
      // Also fetch ratings
      const ratingsResponse = await riderService
      ...
      if (isMountedRef.current) {
        ...
        setStats((prev) => ({
          ...
          totalEarnings:
            totalSummary.totalEarnings || todaySummary.totalEarnings || 0,
          totalRides:
            todaySummary.totalRides ||
            totalSummary.totalRides ||
            prev.totalRides,
          rating: ratingsResponse.success
            ? ratingsResponse.data?.summary?.averageRating || prev.rating
            : prev.rating,
          totalRatings: ratingsResponse.success
            ? ratingsResponse.data?.summary?.totalRatings || 0
            : 0,
          paymentHistory:
            todayData?.earnings?.map((earning) => ({
              id: earning._id,
        ...
        setLiveEarnings(
          todaySummary.walletBalance || totalSummary.walletBalance || 0,
        );
      }
    } catch (error) {
      ...
      if (response.success && isMountedRef.current) {
        ...
        setActiveRide({
          ...
          phone:
            ...
            ride.details?.customerInfo?.phone ||
            ride.details?.user?.phone ||
            "+91 98765 43210",
  ...
  const handleLogout = async () => {
    if (window.confirm("Are you sure you want to logout?")) {
      ...
      try {
        ...
        console.log("Tracking stop error:", e);
      }
      ...
      if (token) {
        fetch(
          `${import.meta.env.VITE_API_URL || "http://localhost:5001"}/api/auth/logout`,
          {
            method: "POST",
            headers: {
  ...
  const fetchAvailableBookings = async () => {
    ...
    try {
      ...
      const riderLng = profileData.currentLocation?.coordinates?.[0] || 77.21;
  ...
  const manualFetchBookings = useCallback(() => {
    if (onlineStatusRef.current && profileData.approvalStatus === "APPROVED") {
      ...
      showToast(
        "You need to be online and approved to refresh bookings",
        "info",
      );
    }
  ...
  const renderActivePage = () => {
    switch (activePage) {
              ...
              if (status === "ongoing" && activeRide) {
                try {
                  ...
                  if (response.success) {
                    ...
                    showToast(
                      response.message || "Failed to start ride",
                      "error",
                    );
                  }
                } catch (error) {
      ...
      case "navigation":
        return (
          <Navigation updateLocation={updateLocation} showToast={showToast} />
        );
      case "documents":
        return (
          <Documents
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-50">
      ...
      <main className="flex-1 overflow-x-hidden">
        ...
        <div className="p-2 sm:p-4 lg:p-6 overflow-x-hidden">
          ...
          <div className="w-full overflow-x-hidden">{renderActivePage()}</div>
        </div>
      </main>
      ...
      {/* Rider Support Modal */}
      {supportOpen && <RiderSupport onClose={() => setSupportOpen(false)} />}
                ...
                notificationList.map((notification, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <h4
                        className={`font-medium ${!notification.isRead ? "text-blue-800" : "text-gray-800"}`}
                      >
