...
//   const [usePickupAsDrop, setUsePickupAsDrop] = useState(false);
...
//       // Set pickup location with coordinates
//       if (heroData.pickup) {
//         setPickupLocation({ 
//           text: heroData.pickup, 
//           lat: heroData.pickupLocation?.lat, 
//           lng: heroData.pickupLocation?.lng 
//         });
//         setManualInput(prev => ({ ...prev, pickup: heroData.pickup }));
//       }
      
//       // Set drop location with coordinates
//       if (heroData.drop) {
//         setDropLocation({ 
//           text: heroData.drop, 
//           lat: heroData.dropLocation?.lat, 
//           lng: heroData.dropLocation?.lng 
//         });
//         setManualInput(prev => ({ ...prev, drop: heroData.drop }));
//       }
      
//       // Set fare details if available
//       if (heroData.fareDetails) {
//         setFareDetails(heroData.fareDetails);
//       }
...
//       // Set trip days for round trip
//       if (heroData.tripDays) {
//         setTripDays(heroData.tripDays);
//       }
...
//         setTimeout(() => navigate('/customer/payment', { state: { bookingData: { bookingId } } }), 1500);
//       }
//     };
...
//         // Update rider location on map if available
...
//         // Always update OTP if available
//         if (booking.otp && !bookingOTP) {
//           setBookingOTP(booking.otp);
//         }

//         // Always fetch rider details if riderId is available
//         if (booking.riderId) {
...
//         // Set pickup location
//         setPickupLocation(locationData);
//         setManualInput({ ...manualInput, pickup: address || 'Your Current Location' });
        
//         // If checkbox is checked, also set drop location
//         if (usePickupAsDrop) {
//           setDropLocation(locationData);
//           setManualInput({ ...manualInput, drop: address || 'Your Current Location' });
//         }
        
//         // Update map view if modal is open
//         if (mapInstanceRef.current && window.google) {
...
//           ? 'Booking created! Driver is being searched...' 
//           : `Scheduled booking confirmed for ${new Date(scheduledAt).toLocaleString()}`
//       );

//       // For CASH payments, navigate to ride tracking page
...
//         });
//       } else {
//         // For online payments, redirect to checkout page first
...
//         <button
//           type="button"
//           onClick={() => { setTripType('ONE_WAY'); setTripDays(1); }}
...
//         <button
//           type="button"
//           onClick={() => { setTripType('ROUND_TRIP'); if (tripDays < 1) setTripDays(1); }}
//           className={`flex-1 py-3 rounded-lg font-medium flex items-center justify-center ${
//             tripType === 'ROUND_TRIP'
//               ? 'bg-yellow-500 text-gray-900'
//               : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
...
//               className="flex-1 py-3 px-4 rounded-lg border border-gray-300 text-center text-lg font-medium focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
//             />
//             <button
//               type="button"
...
//               +
//             </button>
//             <span className="text-gray-600 text-sm">day{tripDays > 1 ? 's' : ''}</span>
//           </div>
//         </div>
//       )}
...
//               disabled={mapLoading}
//               className="p-1.5 text-green-600 hover:bg-green-50 rounded-md mr-1"
//               title="Use current location"
//             >
//               {mapLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Navigation className="h-4 w-4" />}
//             </button>
//             <button
//               type="button"
//               onClick={() => {
...
//         {/* Use same as drop checkbox */}
//         {pickupLocation.lat && (
//           <label className="flex items-center mt-2 cursor-pointer">
//             <input
//               type="checkbox"
//               checked={usePickupAsDrop}
//               onChange={(e) => {
//                 setUsePickupAsDrop(e.target.checked);
//                 if (e.target.checked) {
//                   setDropLocation(pickupLocation);
//                   setManualInput({ ...manualInput, drop: pickupLocation.text });
//                 }
//               }}
//               className="w-4 h-4 text-yellow-500 rounded focus:ring-yellow-500"
//             />
//             <span className="ml-2 text-sm text-gray-600">Use same as drop location</span>
//           </label>
//         )}
//       </div>
...
//                 <span className={`font-medium ${tripType === 'ROUND_TRIP' ? 'text-purple-600' : 'text-green-600'}`}>
//                   {tripType === 'ROUND_TRIP' ? 'Round Trip' : 'One Way'}
//                 </span>
//               </div>
              
//               {tripType === 'ROUND_TRIP' && fareDetails.days && (
//                 <>
//                   <div className="flex justify-between">
//                     <span className="text-gray-600">Number of Days</span>
//                     <span className="font-medium">{fareDetails.days} day{fareDetails.days > 1 ? 's' : ''}</span>
//                   </div>
//                   <div className="flex justify-between">
//                     <span className="text-gray-600">Total Nights</span>
//                     <span className="font-medium">{fareDetails.totalNights} night{fareDetails.totalNights > 1 ? 's' : ''}</span>
//                   </div>
//                 </>
//               )}
...
//                 <span className="font-medium">{fareDetails.distanceKm?.toFixed(1) || 5.0} km</span>
//               </div>

//               {tripType === 'ROUND_TRIP' && fareDetails.billableKm && (
//                 <div className="flex justify-between">
//                   <span className="text-gray-600">Billable KM</span>
//                   <span className="font-medium">{fareDetails.billableKm.toFixed(1)} km</span>
//                 </div>
//               )}
...
//                       <div className="flex justify-between">
//                         <span className="text-gray-600">Driver Allowance ({fareDetails.totalNights} night{fareDetails.totalNights > 1 ? 's' : ''})</span>
//                         <span className="font-medium">₹{fareDetails.driverAllowanceTotal || (fareDetails.driverAllowancePerNight * fareDetails.totalNights) || 0}</span>
//                       </div>
//                     )}
//                   </div>
//                 </>
...
//                   {tripType === 'ROUND_TRIP' 
//                     ? `Includes ${fareDetails.days || 1} day${(fareDetails.days || 1) > 1 ? 's' : ''} driver allowance`
